...
func RegistrationUser(w http.ResponseWriter, r *http.Request) {
	...
	password, err := util.HashPassword(userregistration.Password)
	if err != nil {
		http.Error(w, "Error to bycript the password", http.StatusInternalServerError)
		return
	}
	query := `INSERT INTO users (email,password,verification_code) VALUES (?,?,?)`
